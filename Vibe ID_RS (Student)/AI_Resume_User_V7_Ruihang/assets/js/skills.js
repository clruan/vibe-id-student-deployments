(function () {
  ...
  function renderSkillPills(items, options) {
    ...
    return '<div class="project-skill-strip" aria-label="Related skills">' +
      visible.map(function (entry) {
        return renderSkillToken(entry, selectedKeys.indexOf(entry.key) !== -1, "project-skill-pill", options && options.hideIcons);
      }).join("") +
      (showOverflow ? hidden.map(function (entry) {
        return renderSkillToken(entry, selectedKeys.indexOf(entry.key) !== -1, "project-skill-pill project-skill-pill-extra", options && options.hideIcons);
      }).join("") : "") +
  ...
  function renderSkillToken(entry, selected, extraClass, hideIcon) {
    var color = entry.item.color || "#4f46e5";
    var className = "skill-token" + (extraClass ? " " + extraClass : "") + (selected ? " is-highlighted" : "");
    var icon = !hideIcon && entry.group === "stack" && ns.icons && ns.icons.renderTechIcon
