(function () {
  ...
  function resolveProjectTag(data, label, project) {
    var tagId = normalizeTagId(label);
    var existing = (data.stack || []).find(function (tech) {
      return tech.id === tagId || normalizeTagId(tech.label) === tagId;
    });

    return existing || {
      id: tagId,
      label: label,
      color: project.accent || "#475569",
      projectOnly: true
    };
  }
  ...
  function renderApproach(data, project) {
    ...
    var aiChips = (a.aiStack || []).map(function (label) {
      return ns.icons.renderTechChip(resolveProjectTag(data, label, project), true);
    }).join("");
    ...
    return '<section class="star-card approach-card">' +
      ...
      (aiChips
        ? '<div class="approach-stack-row">' +
            '<div class="approach-stack-group"><span class="approach-stack-label">' + (a.stackLabel || "AI stack") + '</span><div class="approach-stack-chips">' + aiChips + '</div></div>' +
          '</div>'
        : "") +
    '</section>';
  }
  ...
  function renderScreenshotStrip(project) {
    ...
    var html = shots.map(function (shot, idx) {
      if (shot && shot.src) {
        return '<button class="screenshot-thumb" type="button" data-screenshot-index="' + idx + '" aria-label="Open screenshot: ' + (shot.caption || shot.alt || "image " + (idx + 1)) + '">' +
          '<img src="' + shot.src + '" alt="' + (shot.alt || "") + '" loading="lazy" onerror="this.parentNode.classList.add(\'is-missing\')">' +
    ...
    return '<section class="screenshot-strip-section">' +
